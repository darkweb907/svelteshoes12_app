		...
		<h1 class="md:text-4xl text-lg text-center font-semibold mb-4 {isTextVisible ? 'fade-in' : ''}">
			{#if isTextVisible}
				{#each Array.from(typewriterHeading) as char, i}
					<span class="opacity-0 animate-typewriter" style="animation-delay: {i * 0.1}s;"
						>{char}</span
					>
				{/each}
			{/if}
			...
			>
				<div class="words max-sm:text-center max-md:mt-5">
					...
					<p class="text-sm leading-6 text-gray-400">
						Hi there! I'm Segun Odumosu, and I'm passionate about various creative pursuits. I have
						a deep love for crafting beautiful shoes, expressing my soul through singing, and
						enhancing spaces with my interior design skills.
					</p>
